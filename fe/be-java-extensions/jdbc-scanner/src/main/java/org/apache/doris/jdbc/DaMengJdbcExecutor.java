...
public class DaMengJdbcExecutor extends BaseJdbcExecutor {
    private static final Logger LOG = Logger.getLogger(DaMengJdbcExecutor.class);
    private final CharsetDecoder utf8Decoder = StandardCharsets.UTF_8.newDecoder();

    public DaMengJdbcExecutor(byte[] thriftParams) throws Exception {
        super(thriftParams);
    }
    ...
    protected Object getColumnValue(int columnIndex, ColumnType type, String[] replaceStringList) throws SQLException {
        switch (type.getType()) {
            case BOOLEAN:
                Boolean booleanValue = resultSet.getObject(columnIndex + 1, Boolean.class);
                return resultSet.wasNull() ? null : booleanValue;
            case TINYINT:
                Byte byteValue = resultSet.getObject(columnIndex + 1, Byte.class);
                return resultSet.wasNull() ? null : byteValue;
            case SMALLINT:
                Short shortValue = resultSet.getObject(columnIndex + 1, Short.class);
                return resultSet.wasNull() ? null : shortValue;
            case INT:
                Integer intValue = resultSet.getObject(columnIndex + 1, Integer.class);
                return resultSet.wasNull() ? null : intValue;
            case BIGINT:
                Long longValue = resultSet.getObject(columnIndex + 1, Long.class);
                return resultSet.wasNull() ? null : longValue;
            case FLOAT:
                Float floatValue = resultSet.getObject(columnIndex + 1, Float.class);
                return resultSet.wasNull() ? null : floatValue;
            case DOUBLE:
                Double doubleValue = resultSet.getObject(columnIndex + 1, Double.class);
