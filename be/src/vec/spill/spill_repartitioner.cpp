...
Status SpillRepartitioner::repartition(RuntimeState* state,
                                       ...
                                       bool* done) {
    DCHECK_EQ(output_streams.size(), FANOUT);
    ...
    *done = false;
    ...
    std::vector<std::unique_ptr<vectorized::MutableBlock>> output_buffers(FANOUT);
...
Status SpillRepartitioner::_route_block(
    ...
    std::vector<std::vector<uint32_t>> partition_row_indexes(FANOUT);
...
Status SpillRepartitioner::_route_block_by_columns(
    ...
    // Apply SpillPartitionChannelIds: ((hash >> 16) | (hash << 16)) % FANOUT
    for (size_t i = 0; i < rows; ++i) {
        hashes[i] = ((hashes[i] >> 16) | (hashes[i] << 16)) % FANOUT;
    }
