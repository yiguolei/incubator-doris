...
class SpillSortSourceDependency final : public Dependency {
public:
    using SharedState = SpillSortSharedState;
    SpillSortSourceDependency(int id, int node_id, QueryContext* query_ctx)
            : Dependency(id, node_id, "SpillSortSourceDependency", query_ctx) {}
    ~SpillSortSourceDependency() override = default;

    friend class SpillSortSourceOperatorX;
    friend class SpillSortLocalState;
};

class SpillSortLocalState final : public PipelineXLocalState<SpillSortSourceDependency> {
public:
    ENABLE_FACTORY_CREATOR(SpillSortLocalState);
    using Base = PipelineXLocalState<SpillSortSourceDependency>;
...
protected:
    ...
    volatile bool _need_setup_in_memory_op = true;
